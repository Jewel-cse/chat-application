...
const DuplicateButton: React.FC<{
  data: TableData;
  onDuplicate: (data: TableData) => void;
}> = ({ data, onDuplicate }) => {
  ...
  return (
    <button className="mx-1" onClick={handleDuplicate}>
      <FaCopy className="text-custom-blue cursor-pointer" />
    </button>
  );
};
...
export function GlobalTableTwo({
  title,
  data = [],
  columns,
  height,
  ...
  changePageSize,
  dataForNextPage,
  dataForPrevPage, // isNextDisabled,
  // isPrevDisabled,
}: GlobalTableTwoProps) {
  ...
  // const handleRowClick = (rowData: TableData, event: React.MouseEvent) => {
              ...
              {table.getSortedRowModel().rows?.length ? (
                table.getSortedRowModel().rows.map((row, index) => (
                  <TableRow
                    ...
                    // onDoubleClick={() => handleRowClick(row.original, event)}
                  >
                    {/*SL and select row #########################################*/}
                    <TableCell>
                      <div className="flex items-center justify-center">
